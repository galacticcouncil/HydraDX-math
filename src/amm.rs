...
pub fn calculate_out_given_in(in_reserve: Balance, out_reserve: Balance, amount_in: Balance) -> Option<Balance> {
    if out_reserve == 0 || amount_in == 0 {
        return to_u128!(0)
    }

    let (amount_in_hp, out_reserve_hp, in_reserve_hp) = to_u256!(amount_in, out_reserve, in_reserve);
...
pub fn calculate_in_given_out(out_reserve: Balance, in_reserve: Balance, amount_out: Balance) -> Option<Balance> {
    ...
    let (amount_out_hp, out_reserve_hp, in_reserve_hp) = to_u256!(amount_out, out_reserve, in_reserve);
