...
const PRICE_ONE: EmaPrice = (1, 1);
const PRICE_ZERO: EmaPrice = (0, 1);

#[test]
fn saturating_sub_works() {
    assert_eq!(
        saturating_sub(PRICE_ONE, PRICE_ONE),
        (U256::zero(), U256::one())
    );
    assert_eq!(
        saturating_sub((2, 1), PRICE_ONE),
        (U256::one(), U256::one())
    );
    assert_eq!(
        saturating_sub((4, 1), (30, 6)),
        (U256::zero(), U256::from(6))
    );
    assert_eq!(
        saturating_sub((4, 5), (2, 3)),
        (U256::from(2), U256::from(15))
    );
    assert_eq!(
        saturating_sub((1, 10), (2, u128::MAX)),
        (
...
fn weighted_averages_work_on_small_values_with_correct_ratios() {
    ...
    let tolerance = Rational::from((100, u128::MAX));
    let start_price = (4, 100);
    let incoming_price = (8, 100);
    let next_price = price_weighted_average(start_price, incoming_price, smoothing);
    ...
    let start_price = (8, 100);
    let incoming_price = (4, 100);
    let next_price = price_weighted_average(start_price, incoming_price, smoothing);
...
fn price_weighted_average_boundary_values() {
    ...
    let max_price = (1_000_000_000_000_000_000_000_000_u128, 1); // 1e24
    let half_max_price = (1_000_000_000_000_000_000_000_000_u128, 2); // 1e24 / 2

    // previously zero, incoming max
    let next_price = price_weighted_average(PRICE_ZERO, max_price, smoothing);
    assert_rational_relative_approx_eq!(
        Rational::from(next_price),
        Rational::from(half_max_price),
        tolerance
    );
    // previously max, incoming zero
    let next_price = price_weighted_average(max_price, PRICE_ZERO, smoothing);
    assert_rational_relative_approx_eq!(
        Rational::from(next_price),
        Rational::from(half_max_price),
        tolerance
    ...
    let max_price = (1_000_000_000_000_000_000_000_001_u128, 1); // 1e24 + 1
    let half_max_price = (1_000_000_000_000_000_000_000_002_u128, 2); // (1e24 + 2) / 2

    // previously one, incoming max
    let next_price = price_weighted_average(PRICE_ONE, max_price, smoothing);
    assert_rational_relative_approx_eq!(
        Rational::from(next_price),
        Rational::from(half_max_price),
        tolerance
    ...
    let next_price = price_weighted_average(max_price, PRICE_ONE, smoothing);
    assert_rational_relative_approx_eq!(
        Rational::from(next_price),
        Rational::from(half_max_price),
        tolerance
...
fn precision_of_ema_over_price_history_should_be_high_enough_in_crash_scenario() {
    let history = vec![
        (PRICE_ZERO, 1),
        ((1e15 as u128, 1), invariants::MAX_ITERATIONS),
        ((1, 1e15 as u128), invariants::MAX_ITERATIONS),
    ];
    let smoothing = smoothing_from_period(WEEK_PERIOD);
...
fn precision_of_ema_over_balancer_three_months_data_scrape_history_should_be_high_enough() {
    ...
    let history: Vec<(EmaPrice, u32)> = data
        .into_iter()
        .map(|(n, d)| ((n, d), 1_u32))
        .collect();
    let smoothing = smoothing_from_period(WEEK_PERIOD);
