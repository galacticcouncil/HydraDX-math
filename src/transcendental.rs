...
pub fn get_y_constant_product(i: i128, j: i128, new_x: Balance, xp: [Balance; N_COINS_SIZE]) -> Result<Balance, MathError> {

    let x = xp[i as usize];
    let y = xp[j as usize];
    // x * y = new_x * new_y -> new_y = x * y / new_x
    let xy = x.checked_mul(y).ok_or(Overflow)?;
    Ok(xy.checked_div(new_x).ok_or(Overflow)?)
