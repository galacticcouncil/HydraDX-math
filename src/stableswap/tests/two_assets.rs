...
fn test_d() {
    ...
    assert_eq!(calculate_d::<D_ITERATIONS>(&reserves, 1), Some(2000u128 + 2u128));

    let reserves = [1_000_000_000_000_000_000_000u128, 1_000_000_000_000_000_000_000u128];
    assert_eq!(
        calculate_d::<D_ITERATIONS>(&reserves, 1),
...
fn test_y_given_in() {
    ...
    assert_eq!(calculate_d::<D_ITERATIONS>(&reserves, 1), Some(2942u128));
    assert_eq!(
        calculate_y_given_in::<D_ITERATIONS, Y_ITERATIONS>(amount_in, 0, 1, &reserves, 1),
        Some(2000u128 - 121u128)
    );
    assert_eq!(
        calculate_d::<D_ITERATIONS>(&[1100u128, 2000u128 - 125u128], 1),
        Some(2942u128)
    );
}
...
fn test_y_given_out() {
    ...
    assert_eq!(
        calculate_d::<D_ITERATIONS>(&[1000u128 + expected_in, 2000u128 - amount_out], 1),
        Some(2946u128)
    );
}
...
fn test_d_case() {
    ...
    let result = calculate_d::<D_ITERATIONS>(&[500000000000008580273458u128, 10u128], amp);
...
fn test_d_case2() {
    ...
    let result = calculate_d::<D_ITERATIONS>(&[500000000000000000000010u128, 11u128], amp);
